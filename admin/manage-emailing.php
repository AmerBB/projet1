...
if( isset($_POST) ):
    ...
    $query = $db_con->prepare("UPDATE pages SET champ1 = :champ1 , champ3 = :champ3 , champ5 = :champ5 , champ6 = :champ6 , champ7 = :champ7, champ8 = :champ8, champ9 = :champ9 where page='$type' ");    
     ...
     for ($i=1; $i <= 2 ; $i++) :
        ...
        if( !empty( $_FILES['image'.$i]['name'] ) ):
            ...
            if (move_uploaded_file($_FILES['image'.$i]['tmp_name'], $uploadfile)):
                //////////////// Remove Old Image///////////////
                    $fleurscontent = $db_con->prepare("SELECT * FROM pages where page='$type' ");
                    $fleurscontent->execute(); 
                    $content = $fleurscontent->fetch();  
                    if( file_exists( $uploaddir.$content[$champ] ) ) unlink($uploaddir.$content[$champ]);
                ////////////////////////////////////////////////

                $query = $db_con->prepare("UPDATE pages SET   $champ = :champ where page='$type' ");    
